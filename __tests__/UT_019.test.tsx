 ...
 * UT_019: Click Create New Task - Redirect to task creation form
 */
...
describe('UT_019: Click Create New Task - Redirect to task creation form', () => {
  ...
  test('Step 2: Tap "Create New Task"', () => {
    const { getByText } = render(<DashboardScreen navigation={nav} route={{} as any} />);
    const btn = getByText('Create New Task');
    fireEvent.press(btn);
  });
  test('Step 3: Navigation is called with "Create"', () => {
    const { getByText } = render(<DashboardScreen navigation={nav} route={{} as any} />);
    const btn = getByText('Create New Task');
    fireEvent.press(btn);
